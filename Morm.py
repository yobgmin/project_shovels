...
for i in session.query(sec_evt_tbl).filter(sec_evt_tbl.EventID.like('4624')).filter(sec_evt_tbl.SubjectDomainName.like('-')).filter(sec_evt_tbl.LogonType.like('3')):
	...
	if i.LogonType=='3':
		for i in session.query(sec_evt_tbl).filter(sec_evt_tbl.EventID.like('4672')).filter(network_connect_tbl.EventTime.between(i.EventTime+timedelta(seconds=-2), i.EventTime+timedelta(seconds=2))):
