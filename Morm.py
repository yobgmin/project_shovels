...
for i in session.query(proc_tbl).filter(proc_tbl.ParentImage.like('%WmiPrvSE.exe')).filter(proc_tbl.EventID.like('1')):
	print "wmic - Destination", i.EventTime, i.Hostname, i.ParentImage, i.Image, i.CommandLine
	print "\n"
...
for i in session.query(sec_evt_tbl).filter(sec_evt_tbl.EventID.like('4624')).filter(sec_evt_tbl.SubjectDomainName.like('-')).filter(sec_evt_tbl.LogonType.like('3')):
	...
	if i.LogonType=='3':
		for i in session.query(sec_evt_tbl).filter(sec_evt_tbl.EventID.like('4672')).filter(network_connect_tbl.EventTime.between(i.EventTime+timedelta(seconds=-2), i.EventTime+timedelta(seconds=2))).filter(~sec_evt_tbl.Hostname.like(i.Hostname)):
