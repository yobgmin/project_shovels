...
for($x=0;$x<mysqli_num_rows($query_pwdump7);$x++){
	...
	echo "<tr><td>$Image21[$x]</td><td>$msg21[$x]</td><td>$EventTime21[$x]</td></tr>";
}
echo "</table>";
...
for($x=0;$x<mysqli_num_rows($query_winrs3);$x++){
	...
	//echo $query_remoteproc;
	echo "<tr><td>Image</td><td>ParentImage</td><td>Hostname</td><td>EventTime</td></tr>";
	for($y=0;$y<mysqli_num_rows($query_remoteproc);$y++){
		$Temp[$y]=mysqli_fetch_array($query_remoteproc);
	}
	for($y=0;$y<mysqli_num_rows($query_remoteproc);$y++) {
		$Image232[]=$Temp[$y][Image];
		$ParentImage232[]=$Temp[$y][ParentImage];
	...
	echo "<tr><td>$Image23[$x]</td><td>$SourceIP23[$x]</td><td>$DestinationIP23[$x]</td><td>$EventTime[$x]</td></tr>";
}
echo "</table>";
